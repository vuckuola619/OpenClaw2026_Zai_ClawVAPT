...
export class MainOrchestrator {
  ...
  async runRepoSecuritySuite(userId = 'system', profile: RepoScanProfile = 'safe', jobId?: string): Promise<ToolRunResult> {
    ...
    const bridge = await this.engine.bridge.publishAgentEnvelope({ agent: 'RedTeamRepoScannerAgent', task: 'scan', job_id: job.id, status: 'DONE', result: { agent: 'RedTeamRepoScannerAgent', job_id: job.id, status: 'DONE', decision: 'OPENCLAW_CODEX_GPT56_REPO_REVIEW_REQUESTED', evidence: [], findings: result.findings.slice(0, 20), next_state: 'REPO_SCAN_AI_REVIEW_REQUESTED', redaction_applied: true, notes: ['Use OpenClaw Codex GPT-5.6 for advisory public GitHub repo review.'] }, started_at: new Date().toISOString(), completed_at: new Date().toISOString(), redaction_applied: true }, `OpenClaw Codex GPT-5.6 advisory repo scan request. Public GitHub repo only. Job: ${job.id}. Repo: ${job.repoUrl}. Commit: ${job.repoCommit}. Profile: ${profile}. Existing scanner findings: ${result.findings.length}. Return concise security review notes only; no external actions; no private data exfiltration.`);
    result.tools.push({ name: 'OpenClawCodexGPT56Review', status: bridge.status === 'sent' ? 'DONE' : 'INCOMPLETE', available: bridge.status === 'sent', mode: bridge.status === 'sent' ? `openclaw_bridge_run:${bridge.runId || 'started'}` : `openclaw_bridge_${bridge.status}`, notes: [bridge.reason || 'Advisory AI repo review requested through OpenClaw bridge.'] });
