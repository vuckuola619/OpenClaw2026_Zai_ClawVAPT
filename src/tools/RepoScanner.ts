...
export class RepoScanner {
  ...
  async scan(): Promise<Finding[]> { const files = await this.walk(this.root); const findings: Finding[]=[]; const names = new Set(files.map(f=>relative(this.root,f).replace(/\\/g,'/')));
    ...
    for (const f of files.filter(f=>/\.(ts|js|json|yml|yaml|env|Dockerfile|md)$|Dockerfile$/.test(f))) { const rel=relative(this.root,f); const text=await readFile(f,'utf8').catch(()=> ''); const safe = this.redactor.redactString(text);
      ...
      if (/password|api[_-]?key|token|secret/i.test(text) && /[A-Za-z0-9_\-]{24,}/.test(text)) findings.push(this.finding('REPO-SECRET-LIKE','Secret-like pattern redacted','HIGH',`Potential secret-like value detected: ${safe.slice(0,80)}`,rel));
    ...
    return findings; }
  private async walk(dir:string): Promise<string[]> { const entries=await readdir(dir,{withFileTypes:true}).catch(()=>[]); let out:string[]=[]; for(const e of entries){ if(['node_modules','.git','dist','reports','logs','data'].includes(e.name)) continue; const p=join(dir,e.name); if(e.isDirectory()) out=out.concat(await this.walk(p)); else if((await stat(p)).size < 300_000) out.push(p);} return out; }
